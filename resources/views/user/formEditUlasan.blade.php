                    ...
                    <form method="post" action="/review/{{ $review->id }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="id_kamar" id="id_kamar"
                                value="{{ $review->id_kamar }}" required readonly>
                        </div>
                        <div class="form-group">
                            <label>Nilai (1-5)</label><br>
                            @for ($i = 1; $i <= 5; $i++)
                                <input type="radio" id="rating{{ $i }}" name="rating"
                                    value="{{ $i }}" @if ($i == $review->rating) checked @endif required>
                                <label for="rating{{ $i }}">{{ $i }}</label>
                            @endfor
                        </div>
                        <div class="form-group">
                            <label for="comment">Komentar</label>
                            <textarea class="form-control" rows="12" name="comment" id="comment" required>{{ $review->comment }}</textarea>
                        </div>
                        <div class="click">
